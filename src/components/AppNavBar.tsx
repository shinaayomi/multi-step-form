        ...
        {linksList.map((linksContent, index) => (
          <Link href={linksContent.links} key={index} passHref>
            <div className="flex items-center gap-3 cursor-pointer" >
              <div className={`w-8 md:w-10 h-8 md:h-10 grid place-content-center border border-light-blue ${path === linksContent.links ? "bg-light-blue" : "text-light-blue"} font-ubuntu-medium rounded-full`}>
                {index + 1}
              </div>
              <div className='hidden md:block'>
                <p className='text-light-gray text-xs'>STEP {index + 1}</p>
                <p className='text-alabaster font-ubuntu-medium'>{linksContent.title}</p>
              </div>
            </div>
          </Link>
        ))}
